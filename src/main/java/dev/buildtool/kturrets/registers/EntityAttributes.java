...
public class EntityAttributes {
    ...
    public static void registerAttributes(EntityAttributeCreationEvent attributeCreationEvent) {
        ...
        attributeCreationEvent.put(TEntities.BRICK_DRONE.get(), Drone.createDefaultAttributes().add(Attributes.FOLLOW_RANGE, KTurrets.BRICK_TURRET_RANGE.get() - 5).add(Attributes.MAX_HEALTH, Math.max(KTurrets.BRICK_TURRET_HEALTH.get() - 20, 10)).add(Attributes.ARMOR, Math.max(0, KTurrets.BRICK_TURRET_ARMOR.get() - 2)).build());
        attributeCreationEvent.put(TEntities.BULLET_DRONE.get(), Drone.createDefaultAttributes().add(Attributes.FOLLOW_RANGE, KTurrets.BULLET_TURRET_RANGE.get() - 5).add(Attributes.MAX_HEALTH, Math.max(10, KTurrets.BULLET_TURRET_HEALTH.get() - 20)).add(Attributes.ARMOR, Math.max(0, KTurrets.BULLET_TURRET_ARMOR.get() - 2)).build());
