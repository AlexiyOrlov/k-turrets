...
public class EntityAttributes {
    ...
    public static void registerAttributes(EntityAttributeCreationEvent attributeCreationEvent) {
        ...
        attributeCreationEvent.put(TEntities.BRICK_DRONE.get(), Drone.createDefaultAttributes().add(Attributes.FOLLOW_RANGE, KTurrets.BRICK_TURRET_RANGE.get()).add(Attributes.MAX_HEALTH, Math.max(KTurrets.BRICK_TURRET_HEALTH.get() - 20, 10)).add(Attributes.ARMOR, Math.max(0, KTurrets.BRICK_TURRET_ARMOR.get() - 2)).add(Attributes.FLYING_SPEED, 2.4).add(Attributes.MOVEMENT_SPEED, 1.2).build());
    }
}
