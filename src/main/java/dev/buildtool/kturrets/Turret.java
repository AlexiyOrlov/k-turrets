...
public abstract class Turret extends Mob implements RangedAttackMob, MenuProvider, Ownable, Container {
    ...
    public static AttributeSupplier.Builder createDefaultAttributes() {
        return createLivingAttributes().add(Attributes.FLYING_SPEED, 0.27).add(Attributes.MOVEMENT_SPEED, 0.3).add(Attributes.FOLLOW_RANGE, 32).add(Attributes.MOVEMENT_SPEED, 0).add(Attributes.MAX_HEALTH, 60).add(Attributes.ATTACK_DAMAGE, 4).add(Attributes.ARMOR, 3);
    }
    ...
    protected void defineSynchedData() {
        ...
        List<EntityType<?>> targets = new ArrayList<>(ForgeRegistries.ENTITIES.getValues().stream().filter(entityType1 -> !entityType1.getCategory().isFriendly()).toList());
