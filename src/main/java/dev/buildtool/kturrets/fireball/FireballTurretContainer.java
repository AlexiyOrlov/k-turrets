...
public class FireballTurretContainer extends Container2 {
    public FireballTurretContainer(int i, Inventory playerInventory, FriendlyByteBuf buffer) {
        super(KTContainers.FIRE_CHARGE_TURRET.get(), i);
        ...
        addSlot(new ItemHandlerSlot(turret.upgrades, 0,3*18,3*18).setTooltip(Component.translatable("k_turrets.upgrade.slot")).setColor(KTurrets.upgradeSlotColor));
        addSlot(new ItemHandlerSlot(turret.upgrades, 1,4*18,3*18).setTooltip(Component.translatable("k_turrets.upgrade.slot")).setColor(KTurrets.upgradeSlotColor));
        addSlot(new ItemHandlerSlot(turret.upgrades, 2,5*18,3*18).setTooltip(Component.translatable("k_turrets.upgrade.slot")).setColor(KTurrets.upgradeSlotColor));
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (index > KTurrets.turretSlotCount) {
            if (itemStack.is(KTurrets.FIREBALL_UNIT_AMMO) && !moveItemStackTo(itemStack, 0, KTurrets.turretSlotCount -1, false))
