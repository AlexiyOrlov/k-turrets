...
public class BulletDroneContainer extends Container2 {
    public BulletDroneContainer(int i, Inventory playerInventory, FriendlyByteBuf buffer) {
        ...
        addSlot(new ItemHandlerSlot(bulletDrone.upgrades,0,4*18-9,2*18).setColor(KTurrets.upgradeSlotColor).setTooltip(List.of(Component.translatable("k_turrets.lantern.slot"))));
        addSlot(new ItemHandlerSlot(bulletDrone.upgrades,1,5*18-9,2*18).setColor(KTurrets.upgradeSlotColor).setTooltip(List.of(Component.translatable("k_turrets.recall.upgrade.slot"))));
