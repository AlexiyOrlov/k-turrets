...
public class BrickTurretContainer extends Container2 {
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (index > KTurrets.turretSlotCount) {
            if ((itemStack.is( KTurrets.BRICK_UNIT_AMMO_TAG1) || itemStack.is(KTurrets.BRICK_UNIT_AMMO_TAG2)) && !moveItemStackTo(itemStack, 0, KTurrets.turretSlotCount -1, false))
                return ItemStack.EMPTY;
            if((itemStack.is(KItems.EXP_LINK.get()) || itemStack.is(KItems.FIRE_SHIELD.get())) &&!moveItemStackTo(itemStack,KTurrets.turretSlotCount -1,KTurrets.turretSlotCount+KTurrets.turretUpgradeCount,false))
                return ItemStack.EMPTY;
        } else {
